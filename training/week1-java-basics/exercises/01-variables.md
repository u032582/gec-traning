...
課題00では `main` という「決まった入口」を1つ書いただけでした。この課題からは、**自分でメソッドを定義する**ようになります。メソッドは、Javaのすべての処理の基本単位です。

まず、**なぜメソッドという仕組みがあるのか**から。メソッドは、ひとことで言えば「**一度作れば何度でも使い回せる、中身を気にせず呼べるパーツ**」です。うれしさは大きく2つあります。

- **再利用できる（書くのは1回、使うのは何度でも）**: 「あいさつ文を作る」処理を `greeting` というメソッドに1回書いておけば、あとは `greeting("田中")`、`greeting("佐藤")`… と**呼ぶだけ**で何度でも使えます。同じ処理をあちこちにコピペしなくて済むので、直すときも1か所で済みます。
- **中身を意識しなくていい（道具として使える）**: メソッドを呼ぶ側は、**中で何をどうやっているか（処理の中身）を知らなくても使えます**。知る必要があるのは「何を渡せば（引数）／何が返るか（戻り値）」だけ。あなたが普段 `System.out.println(...)` を、中の仕組みを知らずに使えているのと同じです。あれも誰かが作ったメソッドです。

> 💡 この「中身を隠して、入口と出口だけで使わせる」という考え方は、プログラミング全体を貫く一番大事な発想の一つです。大きなシステムも、こうした**信頼できる小さなパーツの組み合わせ**でできています。現場で既存システムに参画するときも、あなたはまず「誰かが作ったメソッドを、中身を全部は読まずに正しく呼ぶ」ことから始めます。
...
> 用語メモ: **メソッド** … 「入力（引数）を受け取り、何か処理して、結果（戻り値）を返す」処理のまとまり。**料理の注文**に似ています。「カレーを・辛口で・大盛りで」のように**いくつでも・種類の違う指定（引数）**を伝えると、その通りに作られた料理（戻り値）が返ってきます。指定する項目の**数も種類も**メソッドごとに決まっていて、`greeting` は「名前を1つ」、`introduce` は「名前と年齢の2つ」を受け取る、という具合です。注文する側が**厨房の調理法を知らなくていい**のも、メソッドと同じです（＝上で書いた「中身を意識しなくていい」）。
