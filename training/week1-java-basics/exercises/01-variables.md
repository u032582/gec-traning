...
> 用語メモ: **メソッド** … 「入力（引数）を受け取り、何か処理して、結果（戻り値）を返す」処理のまとまり。**料理の注文**に似ています。「カレーを・辛口で・大盛りで」のように**いくつでも・種類の違う指定（引数）**を伝えると、その通りに作られた料理（戻り値）が返ってきます。指定する項目の**数も種類も**メソッドごとに決まっていて、`greeting` は「名前を1つ」、`introduce` は「名前と年齢の2つ」を受け取る、という具合です。中の調理法（処理の中身）を知らなくても、何を渡せば何が返るかさえ分かれば使えます。
