...
本物のDBをつなぐと遅い・手間がかかる。Serviceの振る舞いだけを安定してテストできるように、Mapperをモックに差し替えるため。

【when(...).thenReturn(...) は何をしているか】
モックに「このメソッドが呼ばれたら、この値を返す」と仕込む（スタブ）。本物のDBの代わりに、テスト用の戻り値を返させる。

【verify(...) は何をしているか。assertEqualsとの役割の違いは】
verify は、モックのメソッドが本当に呼ばれたか（過程）を確認する。
assertEquals は、Serviceの戻り値が正しいか（結果）を確認する。
```
