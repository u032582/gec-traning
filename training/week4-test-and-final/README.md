...
>
> 🎉 **もう1つの種明かし**: 対になっていた **「更新」**（`bash training/update.sh` ／ 📥 教材を更新）の正体は、**`git fetch origin`（最新を取ってくる）＋ `git merge origin/master`（自分のブランチに取り込む）** でした。これは今週この先で出てくる「**pushする前に origin/master を取り込んで最新化する**」のと、まったく同じ操作です。つまり毎週押していたあのボタンは、**現場で毎日やる「最新に追いつく」動作**の練習だったことになります。以降もボタンを使い続けて構いませんし、中身のコマンドを自分で打っても構いません（[週7の Git/PR おさらい](../week7-safe-modification/README.md#7-git--pr-のおさらい週7以降ずっと共通) でも同じ形が出てきます）。
>
