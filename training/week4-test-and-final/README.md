...
> 用語メモ: **アサーション** … テストの中で「結果はこうなっているはずだ」と**断言する**行のこと。`assertEquals(300, result)` のように、`assert` で始まる呼び出しがそれです。英語の **assert（アサート）＝「断言する」** がそのままカタカナになっただけで、**コードの `assert〜` と同じものを指します**。あとで週8で「アサーション」というカタカナで再登場しますが、別物ではありません。テストで一番大事な行がここです。
...
> 用語メモ: **`./gradlew test` は「いま開いているファイル」だけを実行するのではありません。** そのプロジェクト（`build.gradle` がある階層）の `src/test` に置かれた**テストクラス全部**が対象です。だから自分が2件しかテストを書いていなくても、既にある他のテストも一緒に走り、結果の件数が増えて見えることがあります——それが正常です。特定の1クラスだけ動かしたいときは `./gradlew test --tests "com.example.training.PriceCalculatorTest"` のようにクラスを指定できます。
...
> Cursor（WSL接続状態）でも、内蔵ターミナルを開いて `./gradlew test` を実行できます。テストクラスを開けば各テストの横に出る実行ボタン（▶）からも実行できます。

---

## 6. Git / PR（プルリク）提出の手順 — 初心者向け

> 🎉 **種明かし**: 週1から毎日やってきた **「送信」**（`bash training/send.sh` ／ 📤 成果を送信）の正体は、実は**この節で学ぶ Git の操作**でした。「送信」は内部で、あなたの変更を **commit（記録）** して、あなたのブランチに **push（送る）** していたのです。ここからは、その中身を自分の手で扱えるようになります。さらに今週は、その先の **PR（プルリク＝レビュー依頼）** まで進みます。**送信で地ならしした習慣が、そのまま現場の提出フローにつながります。**
>
> これ以降は、`master` 宛に PR を出してレビューを受ける、という現場の形を練習します（週1〜3で「PRはまだ出さなくていい」と言っていたのは、この週で正式に学ぶからです）。
