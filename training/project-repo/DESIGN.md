...
1. **土台**: Gradle一式（週4のをベースに流用）+ `application.yml` + `schema.sql`（3テーブル+サンプル）。`./gradlew build` が通る空アプリまで。 ← ✅ 済
2. **book 縦切り1本**: 書籍の一覧・1件取得を3層で。`bootRun`→`curl`で動作確認。 ← ✅ 済
3. **member + lending**: 利用者、貸出・返却（在庫の整合を含む業務ルール）。ここがこのアプリの核。 ← ✅ 済
4. **common**: 例外ハンドラ・独自例外で異常系（404・在庫0・二重返却）を整える。 ← ✅ 済（book段階で先行整備）
5. **最低限のテスト**: LendingService の主要分岐だけ。週8で埋める余地を意図的に残す。 ← ✅ 済（3ケース PASS。返却成功/履歴/404系は未カバー）
6. **README.md**: 新人向けに「このアプリは何をするか・どう起動するか・全体像」を書く（週6の入口）。 ← ✅ 済

> **§6 の 1〜6 はすべて完了**。題材リポは「読み・直し・足す」に耐える最小完成形。以降は §7 の残論点（週7の意図的バグ設計・週9〜11の機能追加タスク確定）を、各週の作問時に詰める。
