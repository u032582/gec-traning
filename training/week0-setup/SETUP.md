...
> 📕 **この研修で最初にやるのが、この環境構築です。** 研修ドキュメントは、IDE **Cursor** の **MPE拡張**（Markdownをきれいに表示する機能）で読む想定なので、まず環境を整え、このリポジトリを `git clone` する必要があります。手順の最後（[ステップ5](#ステップ5-リポジトリをcloneしてmpeでドキュメントを表示する)）で clone と MPE 表示まで行います。
>
...
| IDE（Cursor）| **Windows側** | 画面操作はWindowsが快適。WSL2上のコードを開いて使う |
...
## ステップ3: IDE（Cursor）を入れる ＝ Windows側

> 用語メモ: **IDE（アイディーイー）** … コードを書く専用の高機能エディタ。補完やエラー表示、実行ボタンが付いています。この研修では **Cursor（カーソル）** を使います。
> 用語メモ: **Cursor** … AI機能を内蔵したIDE。VS Code をベースに作られていて、ふつうのエディタとしても使え、さらにAIに質問したりコードを書かせたりできます。**この「AI機能」の使い方には研修ルールがあります**（後述。週0〜2はAIに書かせない）。

この研修では **IDEだけWindows側**に入れ、**WSL2上のコードを開いて**使います。

### やること
1. Windowsのブラウザで [cursor.com](https://cursor.com) を開き、Windows版をダウンロード。
2. **Windowsに**インストールして起動。最初の起動でサインインや初期設定を求められたら、画面の指示に従います（無料プランで始められます。研修ではこの無料プランの範囲で十分です）。
3. Cursorが起動できればOK。最初のプロジェクトを開くのは週1で行います。

### WSL2上のコードを開けるようにする（重要）
CursorはVS Code系のIDEなので、**WSL連携の拡張機能**を入れると、WSL2上のフォルダを直接開いて編集できます。
1. Cursorの拡張機能（Extensions）から **「WSL」** を検索してインストール。
2. 左下の青色（または緑色）の `><` のようなボタン（リモート接続）から **「Connect to WSL」** を選ぶと、WSL2(Ubuntu)に接続した状態でIDEが開きます。
3. その状態で「フォルダを開く」→ `/home/（ユーザー名）/` を開けばOKです。
- 週1以降、コードはこの **WSL2側（`/home/ユーザー名/`）** に作り、Cursorで開いて編集 → ビルド・実行は **WSL2のターミナル**（Cursor内のターミナルでも可）で行う、という流れになります。

> 💡 なぜコードをWSL2側に置くのか: Windows側(`C:\...`)にコードを置いてWSL2からビルドすると、ファイルアクセスが非常に遅くなります。**コードはLinux側のホーム（`/home/ユーザー名/`）に置く**のが鉄則です。これは現場でも同じ考え方です。

### ⚠️ 週0〜2は「AI機能をOFF」にして使う（最重要）
Cursorは便利なAI機能を持っていますが、**週0〜2（基礎フェーズ）ではコードを書かせる使い方は禁止**です（理由は[研修トップのAI利用ルール](../README.md#2-ai利用ルール最重要必ず守る)を参照）。Cursorの主なAI機能は次の3つです。基礎期は次の設定で使ってください。

- 🚫 **Composer / Agent（エージェント）モード** … 使わない。指示するとファイルを自動で書き換えてしまう機能なので、基礎期は触りません。
- 🚫 **Tab補完**（タイピング中に続きを灰色の文字で勝手に提案し、Tabキーで確定する機能）… **OFFにする**。これがONだと、自分で考える前に答えが出てしまい、基礎力が付きません。設定で `Cursor Tab` を無効化してください（コマンドパレットで `Cursor: Toggle Cursor Tab` を実行するか、右下のステータスバーの Cursor Tab アイコンから切り替えられます）。やり方が分からなければChatに「CursorのTab補完をオフにする方法」を聞いてOKです。
- ✅ **Chat（チャットで質問）モード** … **使ってよい**。これは「家庭教師」用途（エラーの意味を聞く・自分のコードのレビュー・概念説明）に当たり、週0〜2でも許可されています。Chatは質問に答えるだけで、あなたのコードを勝手に書き換えません（自分から「適用」しない限り反映されません）。

週3以降（AI協働フェーズ）になったら、Composer/AgentもTab補完もONにして、AI活用を解禁します（ただし「生成コードを自分の言葉で説明できること」が条件）。
...
<details>
<summary>CursorがJDKを見つけてくれない</summary>

WSL2側に入れたJDKをIDEに教える必要がある場合があります。WSLに接続した状態でプロジェクトを開いていれば、WSL2側のJDK21が使われます。うまくいかない場合はChatに「CursorでWSL2上のJDK21をプロジェクトに設定する手順」を聞いてください。週1の最初に詳しく扱います。
</details>

<details>
<summary>IntelliJ / Eclipse を使いたい</summary>

この研修の標準IDEはCursorです。慣れている人がIntelliJ等を使うこと自体は妨げませんが、手順書はCursor前提で書かれています。IntelliJ/Eclipseを使う場合も「IDEはWindows側・コードとビルドはWSL2側」の原則は同じです。
</details>
...
ここまでで道具がそろいました。最後に、**この研修ドキュメント自体を、読みやすい形で表示できるようにします。** この研修の資料は、Cursor の **MPE拡張**（Markdownをきれいに整形して表示する機能）で読む想定です。
...
### 2. Cursorでそのフォルダを開く（WSL接続状態で）
1. Cursorを起動し、左下の `><`（リモート接続）から **「Connect to WSL」** を選ぶ。
2. 「フォルダを開く」で、cloneした研修フォルダ（例: `/home/ユーザー名/gec-traning`）を開く。

### 3. MPE拡張を入れる
1. Cursorの拡張機能（Extensions）で **「Markdown Preview Enhanced」** を検索してインストール。
...
WSL接続状態で拡張を入れているか確認してください（ローカルだけに入れてもWSL側のファイルには効かないことがあります）。拡張のページに「Install in WSL: Ubuntu」ボタンが出ていたら押します。それでもダメなら、いったんCursorを再起動してから再度プレビューを試してください。
</details>

---

## ステップ6: 「送信」を1回ためして、先生に見せられる状態にする

研修では、1日の終わりに**その日書いたコードを先生が見られる場所へ送る**「送信（そうしん）」という習慣があります（詳しくは [研修トップREADME](../README.md) の「1. 進め方 → 毎日の終わりに『成果を送信』する」）。ここでは、**その送信が最後まで通るか**を1回だけ確かめておきます（まだ成果は無くてOK。仕組みを1本通すだけです）。

> なぜ週0でやるか: 送信には「ネット上の置き場（GitHub等）にログインできている（認証）」ことが要ります。それが通るかを、課題が始まる前に一度だけ確認しておくと、あとで安心して毎日送れます。

### やること（WSL2のターミナルで、cloneした研修フォルダの中で）
```bash
cd ~/gec-traning        # cloneした研修フォルダへ（フォルダ名は各自のものに）
bash training/send.sh    # ← これが「送信」。初回だけ自分のハンドル（英小文字の短い名前）を聞かれます
```
- ハンドルは、あなた専用の置き場所の名前になります（例 `tanaka`）。ローマ字でOK。
- 最後に **「🎉 送信できました！」** と、あなた専用ページのURLが出れば成功です。そのURLをブラウザで開くと、GitHub上に**あなたのブランチ**ができているのが見えます。

> 💡 いまはまだコードを書いていないので「変更はありませんでした」と出ますが、それで正常です。**大事なのは、送信（push）が最後まで通ったこと**。週1からは、1日の終わりにこの送信をするのが習慣になります。中身（Git）は週4で学ぶので、今は「送信＝先生に見せるボタン」で大丈夫です。

> 📥 **もう1つ、対になるボタンがあります**: この教材は、みなさんの日報を読んで直り続けます。その直しを手元に取り込むのが **「📥 教材を更新（先生の直しを取り込む）」**（`bash training/update.sh`）です。送信が「あなた → 先生」なら、更新は「先生 → あなた」の向き。**週のはじめに1回押す**くらいの頻度で構いません。詳しくは [研修トップREADME](../README.md) の「1. 進め方 → ときどき『教材を更新』する」。

### つまずきポイント
<details>
<summary>「送信（push）に失敗しました」／認証・Permission denied が出る</summary>

多くは、GitHubへのログイン（認証）がまだ通っていないためです。cloneのときと同じで、非公開リポジトリはアクセス権が必要です。講師から共有された方法（HTTPSのURL＋トークン、またはSSHの設定）に従ってください。エラー全文をAIのAskモードに貼って「このgit pushのエラーの意味と対処を初心者向けに教えて」と聞き、それでも分からなければ質問テンプレートで講師に相談を。
</details>
...
- [ ] Cursor（Windows側）が起動し、WSL連携でWSL2上の `/home/（ユーザー名）` を開ける
...
- [ ] CursorでcloneしたフォルダをWSL接続で開き、MPE拡張で `training/README.md` をプレビュー表示できた
- [ ] `bash training/send.sh`（送信）が最後まで通り、「🎉 送信できました！」と自分のブランチのURLが出た
...
```
2. `Hello.java` を作ります。Cursor（WSL接続状態）で `/home/ユーザー名/hello` を開いて作ってもよいですし、ターミナルで次のように作っても構いません。
```bash
cat > Hello.java <<'EOF'
public class Hello {
...
> 💡 Cursorで実行する場合は、WSL接続した状態でこのフォルダを開き、`Hello.java` の `main` 横に出る実行ボタン（▶）から実行できます（JDKはWSL2側の21が使われます）。どちらの方法でもこの文字が出れば、あなたの開発環境は完全に整っています。
...
3. Cursor（Windows側）を起動し、WSL連携でWSL2上のプロジェクトを開く。
